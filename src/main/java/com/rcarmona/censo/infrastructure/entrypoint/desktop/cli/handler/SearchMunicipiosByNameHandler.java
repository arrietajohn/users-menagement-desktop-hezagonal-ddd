...
/**
 * Handler para buscar Municipios por nombre.
 * 
 ...
 * - Controlador interactivo de consola. Intercepta la acción de SearchMunicipiosByName, solicita la entrada de datos al usuario de forma amigable, ensambla la petición y delega la ejecución al Controlador Principal.
 * 
 * Detalles del Componente:
 * - Nombre de Clase/Interface: SearchMunicipiosByNameHandler
 ...
 * ==========================================================================================
 */
public class SearchMunicipiosByNameHandler implements MunicipioOperationHandler {

    private final SearchMunicipiosByNameUseCase useCase;
    private final CensoConsoleIO console;

    @Override
    public void handle() {
        console.println("\n--- Buscar por Nombre ---");
        String partial = console.readRequired("Parte del nombre: ");
        
        List<MunicipioModel> models = useCase.execute(partial);
        ...
        if (responses.isEmpty()) {
            console.println("No se encontraron municipios con ese nombre.");
        } else {
