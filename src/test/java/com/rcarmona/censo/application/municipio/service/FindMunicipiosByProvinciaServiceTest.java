 ...
 * - Suite de pruebas unitarias (Testing). Valida automáticamente el comportamiento y las reglas de negocio de la clase FindMunicipiosByProvinciaService, garantizando que no existan regresiones en el código mediante escenarios de éxito y fallo.
 * 
 * Detalles del Componente:
 * - Nombre de Clase/Interface: FindMunicipiosByProvinciaServiceTest
 ...
 * ==========================================================================================
 */
class FindMunicipiosByProvinciaServiceTest {

    @Mock
    private FindMunicipiosByProvinciaPort port;

    @InjectMocks
    private FindMunicipiosByProvinciaService service;

    @Test
    void execute_ReturnsList() {
        MunicipioModel model = new MunicipioModel(new MunicipioId(1), "Cartagena", 13);
        when(port.findByProvinciaId(13)).thenReturn(List.of(model));

        List<MunicipioModel> result = service.execute(13);

        assertFalse(result.isEmpty());
        assertEquals(13, result.get(0).getProvinciaId());
    }
}
