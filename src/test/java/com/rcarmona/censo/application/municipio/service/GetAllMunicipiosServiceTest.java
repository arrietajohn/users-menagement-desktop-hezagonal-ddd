 ...
 * - Suite de pruebas unitarias (Testing). Valida automáticamente el comportamiento y las reglas de negocio de la clase GetAllMunicipiosService, garantizando que no existan regresiones en el código mediante escenarios de éxito y fallo.
 * 
 * Detalles del Componente:
 * - Nombre de Clase/Interface: GetAllMunicipiosServiceTest
 ...
 * ==========================================================================================
 */
class GetAllMunicipiosServiceTest {

    @Mock
    private GetAllMunicipiosPort port;

    @InjectMocks
    private GetAllMunicipiosService service;

    @Test
    void execute_ReturnsList() {
        MunicipioModel model = new MunicipioModel(new MunicipioId(1), "Cartagena", 13);
        when(port.findAll()).thenReturn(List.of(model));

        List<MunicipioModel> result = service.execute();
